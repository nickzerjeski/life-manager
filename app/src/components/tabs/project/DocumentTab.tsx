...
const DocumentTab: React.FC<DocumentTabProps> = ({ project }) => {
              ...
              <div className="flex items-center space-x-2 flex-grow min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-gray-600">
                    Type: {doc.type} | Uploaded: {doc.uploadDate.toISOString()}
                  </p>
                </div>
              </div>
              <button
      ...
      {activeDoc && (
        <Modal isOpen={!!activeDoc} onClose={closeModal} title={activeDoc.name}>
          {activeDoc.type === 'pdf' && activeContent ? (
            <iframe src={`data:application/pdf;base64,${activeContent}`} className="w-full h-[80vh]" />
